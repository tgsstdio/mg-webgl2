...
export function pipelineTest (backbone: MgBackbone) {
  ...
  vsCreateInfo.code = getShaderSource('vs-old');
  ...
  positionBinding.stride = 8;
  positionBinding.inputRate = MgVertexInputRate.VERTEX;

  let colorBinding = new MgVertexInputBindingDescription();
  colorBinding.binding = 1;
  colorBinding.stride = 12;
  colorBinding.inputRate = MgVertexInputRate.INSTANCE;

  let positionAttribute = new MgVertexInputAttributeDescription();
  positionAttribute.binding = 0;
  positionAttribute.format = MgFormat.R32G32_SFLOAT;
