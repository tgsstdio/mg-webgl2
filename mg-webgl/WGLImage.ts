...
namespace Magnesium {
  export class WGLImage implements IWGLImage {
    ...
    private mArraySubresources: Array<GLImageArraySubresource>;  
    get arrayLayers(): Array<GLImageArraySubresource> {
    ...
    private generateMipmaps() : void {
      ...
      // WARN: imageSize requires ulong
			...
			this.mArraySubresources = new Array<GLImageArraySubresource>(this.mLayers);

			for (let index = 0; index < this.mLayers; index += 1) {
				let arrayItem = new GLImageArraySubresource(
          index
          , new Array<GLImageLevelSubresource>(this.mLevels));
