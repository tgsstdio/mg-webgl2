...
import {WGLUniformBlockGroupInfo}
	from './WGLUniformBlockGroupInfo';
import {IWGLPipelineLayout}
	from './IWGLPipelineLayout';	
import {WGLUniformBlockGroupCollator}
	from './WGLUniformBlockGroupCollator';	  
  ...
  calculateArrayIndex(
    ...
    if (this.mGroups.has(entry.firstBinding)) {
      ...
      if (this.mLayout.ranges.has(entry.firstBinding)) {
        let arrayOffset = this.mLayout.ranges.get(entry.firstBinding) as WGLBindingPointOffsetInfo;
