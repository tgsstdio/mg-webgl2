...
import {IMgEntrypoint} from './IMgEntrypoint'
import {MgApplicationInfo} from './MgApplicationInfo'
import {IMgInstance} from './IMgInstance'
import {MgResult} from './MgResult'
import {MgInstanceCreateInfo} from './MgInstanceCreateInfo'
import {MgInstanceExtensionOptions} from './MgInstanceExtensionOptions'
...
import {MgQueueInfo} from './MgQueueInfo'
import {MgLogicalDevice} from './MgLogicalDevice'
...
export class MgDriverContext implements IMgDisposable {
  ...
  createLogicalDevice(
    ...
    let out_0
      : { physicalDevices: Array<IMgPhysicalDevice> | null }
    ...
    let first 
      : IMgPhysicalDevice
      = (out_0.physicalDevices as Array<IMgPhysicalDevice>)[0];

    let enabledExtensions = new Array<string>(); 
    if (option == MgDeviceExtensionOptions.ALL) {
      let out_1
        : { pProperties: Array<MgExtensionProperties> | null }
        = { pProperties: null };
  ...
  private createDeviceA(
    ...
    let out
      : { pQueueFamilyProperties: Array<MgQueueFamilyProperties> | null }
